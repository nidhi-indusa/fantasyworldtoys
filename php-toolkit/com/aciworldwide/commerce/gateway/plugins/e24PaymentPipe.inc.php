...
public function __construct() 
  ...
  function performTransaction() {
    ...
    if ($arraylist == null) {
	  ...
	  $this->responseCode = $arraylist[12];
      return 0;
    }
  }
  ...
  function sendMessage($s, $s1) {
	...
	try
	{
      ...
      if (strlen($s) > 0) {
        ...
        $tmp = curl_exec($c);
		if (curl_errno($c)) {
			var_dump(curl_errno($c), curl_error($c));
		}
  ...
  function parseSettings($s) {
	...
	// $parseData .= 'Id: ' . substr($s, $i, $j - $i);	// Added
	...
	// $parseData .= ', Password: ' . substr($s, $i, $j - $i);	//Added
    $i = strpos($s, "<webaddress>") + strlen("<webaddress>");
    $j = strpos($s, "</webaddress>");
    $this->setWebAddress(substr($s, $i, $j - $i));
	// $parseData .= ', WebAddr: ' . substr($s, $i, $j - $i);	//Added
    $i = strpos($s, "<port>") + strlen("<port>");
    $j = strpos($s, "</port>");
    $this->setPort(substr($s, $i, $j - $i));
	// $parseData .= ', Port: ' . substr($s, $i, $j - $i);	//Added
    $i = strpos($s, "<context>") + strlen("<context>");
    $j = strpos($s, "</context>");
    $this->setContext(substr($s, $i, $j - $i));
	// $parseData .= ', Context: ' . substr($s, $i, $j - $i);	//Added

	
	// $filenameOutput = $this->getResourcePath() . "decryptData.txt";
	// $handleOutput = fopen($filenameOutput, "w");
	// fwrite($handleOutput, $s);      
	// fclose($handleOutput);
